...
export default function InvestorsRelations() {
    ...
    const [formData, setFormData] = useState({ headline: '', pdf: null });
    ...
    const fetchReports = async () => {
        try {
            ...
            toast.error('Failed to load investors relations reports');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            let response;
            if (editingId) {
                response = await axios.put(`${API_BASE}/api/investors-relations/${editingId}`, data, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
            } else {
                response = await axios.post(`${API_BASE}/api/investors-relations`, data, {
                    headers: { 'Content-Type': 'multipart/form-data' }
                });
            }
    ...
    const handleEdit = (report) => {
        ...
        setFormData({ headline: report.headline, pdf: null });
        setIsModalOpen(true);
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            const response = await axios.delete(`${API_BASE}/api/investors-relations/${reportToDelete}`);
            if (response.data.success) {
        ...
        <div className="p-6 md:p-8 bg-slate-50 min-h-screen">
            ...
            <div className="max-w-6xl mx-auto">
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <h1 className="text-2xl md:text-3xl font-bold text-slate-800">Investor's Relations</h1>
                        <p className="text-slate-500 mt-1">Manage valuation reports and disclosures</p>
                    </div>
                    <button
                        onClick={() => {
                            setEditingId(null);
