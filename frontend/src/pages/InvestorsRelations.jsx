...
export default function InvestorsRelations() {
    ...
    useEffect(() => {
        const fetchReports = async () => {
            try {
                const response = await axios.get(`${API_BASE}/api/investors-relations`);
                if (response.data.success) {
                    setReports([...response.data.data].reverse());
                }
            } catch (error) {
                console.error("Failed to fetch investor relations:", error);
            } finally {
                setLoading(false);
            }
        };
            ...
            <main className="flex-grow flex items-center justify-center py-7 px-4 sm:py-10 sm:px-6 lg:py-12 lg:px-8 bg-slate-50/50">
                <div className="w-full max-w-4xl lg:max-w-5xl xl:max-w-6xl animate-fade-in-up">
                    <div className="mb-6 md:mb-8 text-center transition-all duration-500 hover:scale-[1.01]">
                        <h1 className="text-2xl text-left md:text-3xl lg:text-4xl font-bold text-slate-900 mb-2 tracking-tight">Investor's <span className="text-[#50ad77]">Relations</span></h1>
                        <p className="text-sm text-left md:text-base text-slate-600">Explore our latest valuation reports and disclosures.</p>
                    </div>
                    <div className="bg-white rounded-2xl shadow-sm border border-slate-200/80 overflow-hidden transition-shadow duration-300 hover:shadow-md">
                        <div className="bg-slate-50/50 px-4 md:px-6 py-3 md:py-4 border-b border-slate-200 flex items-center justify-between">
                            <h2 className="text-xs md:text-sm font-bold text-slate-700 flex items-center space-x-2">
                                ...
                                <span>Available Disclosures ({reports.length})</span>
                            </h2>
                        </div>
                                                    ...
                                                    >
                                                        ...
                                                        <td className="py-3 lg:py-4 px-4 lg:px-6">
                                                            <div className="flex items-start space-x-3">
                                                                <div>
                                                                    <h4 className="text-xs lg:text-sm font-semibold text-slate-900 group-hover:text-slate-950 transition-colors duration-300 leading-tight">
                                                                        {report.headline}
                                                                    </h4>
                                                                </div>
                                                            </div>
                                                        ...
                                                        <td className="py-3 lg:py-4 px-4 lg:px-6 text-right">
                                                            {isSimulating ? (
                                                                <div className="inline-flex items-center justify-end space-x-2 animate-fade-in">
                                                                    ...
                                                                    <span className="text-[10px] lg:text-xs font-bold text-[#50ad77]">{downloadProgress}%</span>
                                                                </div>
                                                            ) : (
                                                                <button
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="flex items-start space-x-3.5 mb-4">
                                                    <span className="p-2 rounded-lg bg-[#eef7f2] text-[#50ad77] mt-0.5 flex-shrink-0 transition-transform duration-300 group-hover:scale-110">
                                                        <FileIcon />
                                                    </span>
                                                ...
                                                <div>
                                                    {isSimulating ? (
                                                        <div className="w-full bg-[#eef7f2] border border-[#50ad77]/20 rounded-xl py-3 flex items-center justify-center space-x-3 animate-fade-in">
                                                            ...
                                                            <span className="text-xs font-bold text-[#50ad77]">Downloading ({downloadProgress}%)</span>
                                                        </div>
                                                    ) : (
                                                        <button
                                                            onClick={() => triggerDownload(report)}
