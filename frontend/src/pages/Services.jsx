    ...
    <div className="min-h-screen bg-[#f8fafc] font-sans text-slate-800 overflow-x-hidden">
      ...
      <div className="relative h-40 sm:h-56 md:h-64 lg:h-72 w-full bg-slate-900 overflow-hidden animate-fade-in">
        ...
        <div className="absolute inset-0 z-20 flex flex-col items-center justify-center text-center px-4">
          ...
          <p className="text-slate-300 text-[11px] sm:text-sm mt-1 sm:mt-2 max-w-xl font-light animate-fade-in" style={{ animationDelay: '200ms' }}>
            <Link to="/" className="hover:text-white transition-colors duration-300">Home</Link>{" "}/ Services
          </p>
        </div>
      </div>
      <section className="w-full max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12 lg:py-16">
        {loading ? (
          ...
          <>
            <div className="flex flex-col gap-4 sm:flex-col sm:gap-5 md:flex-row md:justify-between md:items-start md:gap-6 mb-8 sm:mb-10 lg:mb-14 animate-fade-in-up">
              <div className="flex-1 min-w-0">
                <h2 className="font-extrabold text-[#1a233a] tracking-tight mb-2 sm:mb-3 text-2xl sm:text-3xl lg:text-4xl truncate">{filteredServices[0]?.title || ''}</h2>
              ...
              <div className="w-full flex-shrink-0 sm:w-full md:w-64 lg:w-72">
                <p className="font-semibold uppercase tracking-widest text-gray-700 mb-1.5 text-[10px] sm:text-xs lg:text-xs text-center">Choose Services</p>
                <div className="relative">
                  <select className="w-full appearance-none bg-white border border-gray-200 text-gray-700 rounded-xl sm:rounded-2xl shadow-sm font-semibold cursor-pointer transition-all focus:outline-none focus:ring-2 focus:ring-[#50ad77]/50 focus:border-[#50ad77] hover:shadow-md py-2.5 pl-4 pr-9 text-sm sm:py-3 sm:pl-5 sm:pr-10 lg:text-base" value={selectedServiceId || ''} onChange={(e) => setSelectedServiceId(e.target.value)}>
                    {servicesData.map(s => (
            ...
            {filteredServices.length === 0 ? (
              <div className="text-center py-20 text-slate-500 text-sm sm:text-base animate-fade-in">No services found.</div>
            ) : (
              <div className="space-y-10 sm:space-y-14 lg:space-y-20">
                {filteredServices.map((service, index) => (
                  <div key={service.id} className="animate-fade-in-up" style={{ animationDelay: `${index * 100}ms` }}>
                    <div className="w-full rounded-xl sm:rounded-2xl lg:rounded-3xl shadow-md sm:shadow-lg overflow-hidden bg-white">
                      <img src={`${API_BASE}/uploads/${service.image}`} alt={service.title} className="block w-full h-auto object-contain" />
                    </div>
                    <div className="mt-4 sm:mt-6 lg:mt-8 px-0.5 sm:px-1 lg:px-2">
                      <h3 className="flex items-center gap-2 capitalize font-bold text-gray-900 mb-2 sm:mb-3 lg:mb-4 text-base sm:text-xl lg:text-2xl">
                        <svg xmlns="http://www.w3.org/2000/svg" className="flex-shrink-0 text-[#50ad77] h-4 w-4 sm:h-5 sm:w-5 lg:h-6 lg:w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z" />
                        </svg>
                        {service.title}
                      </h3>
                      <p className="text-gray-600 leading-relaxed whitespace-pre-wrap text-sm sm:text-base lg:text-lg">{service.description}</p>
                    </div>
                  </div>
                ))}
              </div>
