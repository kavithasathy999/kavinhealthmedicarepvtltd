...
exports.addResourceRepository = async (req, res) => {
    try {
        ...
        const query = "INSERT INTO resource_repository (headline, pdf_url, file_size, report_year) VALUES (?, ?, ?, ?)";
...
exports.updateResourceRepository = async (req, res) => {
    try {
        ...
        if (pdf_url) {
            ...
            query = "UPDATE resource_repository SET headline=?, pdf_url=?, file_size=?, report_year=? WHERE id=?";
            values = [headline, pdf_url, file_size, report_year || null, id];
        } else {
            query = "UPDATE resource_repository SET headline=?, report_year=? WHERE id=?";
